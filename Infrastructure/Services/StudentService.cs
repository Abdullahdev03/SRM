  ...
  // TODO: Abdulloh
     ...
     public async Task<Response<StudentDto>> GetStudentById(int id)
    {
     ...
     public async Task<Response<List<StudentDto>>> GetStudentByName(string name)
     {
         ...
         var students = await _context.Students.Where(s => s.FirstName.Contains(name)).Select(x=>new StudentDto()
         {
             Id = x.Id,
             FirstName = x.FirstName,
             LastName = x.LastName,
    ...
    public async Task<Response<StudentDto>> UpdateStudent(StudentDto student)
    {
        ...
        var existing = await _context.Students.FindAsync(student.FirstName);
